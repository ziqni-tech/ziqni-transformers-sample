...
//import com.ziqni.transformers.domain.{ZiqniEntityChangeSubscriptionRequest, ZiqniEntityChanged, ZiqniEntityStateChanged, ZiqniEvent}
...
//					val jackpotWinEvent = ZiqniEvent(
//						memberId = memberId,
...
//					val additionalJackpotWinEvent = ZiqniEvent(
//						memberId = memberId,
...
//					val winMultiplierEvent = ZiqniEvent(
//						memberId = memberId,
...
//				case _ =>
//					ziqniContext.ziqniApiAsync.createMember(CreateMemberRequest(memberRef, displayName.getOrElse(memberRef), groups.distinct, memberMeta))
//			}} yield out
//	}
//
//
//	// handle action creation inside in action helpers
//	private def getOrCreateEventAction(action: String, ziqniContext: ZiqniContext, basicUnitOfMeasureKey: Option[String]): Unit = {
//		implicit val context: ExecutionContextExecutor = ziqniContext.ziqniExecutionContext
//
//		ziqniContext.ziqniApiAsync.eventActionExists(action.toLowerCase).map(exists => {
//			if (!exists)
//				ziqniContext.ziqniApiAsync.createEventAction(CreateEventActionRequest(action.toLowerCase, None, None, basicUnitOfMeasureKey))
//		})
//	}
//
//	/**
...
//			.map(event => event.filter(x => x.action == BET_ACTION))
//			.map(betEvents => {
...
//	 * @return
//	 */
//	override def getEntityChangeSubscriptionRequest(ziqniContext: ZiqniContext): Seq[ZiqniEntityChangeSubscriptionRequest] = webHookSettings.classicEntityChangeSubscriptionRequest
//
//	override def onEntityChanged(change: ZiqniEntityChanged, ziqniContext: ZiqniContext): Unit = {
//		super.onCustomEntityChanged(webHookSettings, change,ziqniContext)
//	}
//
//	override def onEntityStateChanged(change: ZiqniEntityStateChanged, ziqniContext: ZiqniContext): Unit = {
